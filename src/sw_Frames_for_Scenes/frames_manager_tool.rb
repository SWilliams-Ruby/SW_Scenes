module SW
  module FramesForScenes
    module FramesManager
      ...
      # Sketchup tool interface methods to be included in a user's
      # custom tool
      class FramesManagerTool
        
        def initialize(frames_manager)
          @frames_manager = frames_manager
          @default_cursor = UI.create_cursor(File.join(SW::Scenes::PLUGIN_DIR, 'sw_Frames_for_Scenes', "cursors", 'pointer.png'), 1, 7)
        end
        ...
        def activate
          ...
          @timer = UI.start_timer(0.5, true) {hide()}
        end
